...
****************************************************************************/
{
   ...
   public void Cmd_D(File_Buffer FILE, int nLines)
   {
       ...
       if(Valid_Lines(FILE,nLines)) 
       {
          ...
          for(i=CLN; i<=end; i++) //여기 고쳐야한다 i<=end로
              FILE.DelLine(CLN);
   ...
   public void Cmd_F(File_Buffer FILE, int nLines, String findstr)
   {
       ...
       //for(i=CLN; i<=end; i++)  //i<end 까지 수행되어야한다.
   ...
   public void Cmd_I(File_Buffer FILE)
   {
       ...
       for(i=1; i<=nlines; i++) 
       {
        ...
        //      break;						//break때문에 한문장하면 다음 문장을 수행하지 않으므로 코드 수정 필요.

          if(aline.length()==0||aline.charAt(0) != '@')    //수정 후 코드.
             continue; 
          else                               // Extract keyword and store it
          {
             ...
             System.out.println(aline.substring(fpos,lpos+1));
   ...
   public void Cmd_K(String keyword)
   {
       ...
       for(i=1; i<=num_kw; i++) 
       {
           ...
           //if((kwRec.getKW()).compareTo(keyword) == 0)  //compareTo메소드를 사용하면 문장 전체를 키워드로 입력해야하므로 코드 수정 필요.
           if((kwRec.getKW()).contains(keyword))  //수정 후
           {
              ...
              Msg.wLMsg("");
              found=true;
             // break; //파일에 같은 단어가 2개 이상 존재하면 1개밖에 찾지못하기 때문에 코드 수정 필요.
              continue; //수정 후
           }
       }
